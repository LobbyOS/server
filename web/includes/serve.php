...
foreach($files as $file){
  ...
  if($file == "/includes/lib/jquery/jquery-ui.js" || $file == "/includes/lib/jquery/jquery.js" || $file == "/includes/lib/lobby/js/main.js" || $file == "/includes/lib/core/JS/app.js"){
  ...
  if(isset($css)){
    $to_replace = array(
      "<?phpL_URL?>" => L_URL,
      "<?phpTHEME_URL?>" => THEME_URL
    );
    if(isset($_GET['APP_URL'])){
      $to_replace["<?phpAPP_URL?>"] = htmlspecialchars(urldecode($_GET['APP_URL']));
      $to_replace["<?phpAPP_SRC?>"] = htmlspecialchars(urldecode($_GET['APP_SRC']));
    }
