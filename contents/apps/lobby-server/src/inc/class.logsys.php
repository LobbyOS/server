  ...
  public static function login($username, $password, $remember_me = false, $cookies = true){
    ...
    if(self::$db === true){
      ...
      if(self::$config['features']['email_login'] === true){
        $query = "SELECT `id` FROM `". self::$config['db']['table'] ."` WHERE `username`=:login OR `email`=:login ORDER BY `id` LIMIT 1";
      }else{
        $query = "SELECT `id` FROM `". self::$config['db']['table'] ."` WHERE `username`=:login ORDER BY `id` LIMIT 1";
      }
  ...
  public static function register( $id, $password, $other = array() ){
    ...
    if( self::userExists($id) || (isset($other['email']) && self::userExists($other['email'])) ){
      ...
      if( count($other) == 0 ){
        ...
        $sql   = self::$dbh->prepare("INSERT INTO `". self::$config['db']['table'] ."` (`username`, $columns) VALUES(:username, :$colVals)");
